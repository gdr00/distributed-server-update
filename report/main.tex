...
(Consistency and Partition tolerance part of the CAP theorem\cite{gilbert2002}; ACID principle): since RAFT\cite{raft}
based, when the leader or the majority ($N/2+1$ nodes) is unreachable,
there is no working system (not always Available; CAP\cite{gilbert2002}). For configuration data, human operators can intervene
...
(AP\cite{gilbert2002})} {\color{violet}at the cost of strong} Consistency.
...
{\color{violet}The store is a compacted \textbf{LWW-element-set} \cite{shapiro2011} keyed by
  setting name. Rather than maintaining separate add/remove sets, each element only stores
  their merge result: the highest-timestamp operation plus a tombstone flag (\texttt{Deleted}).
  A live element carries its value. Write and delete are timestamped operations,
  on merge the higher timestamp wins and the flag selects whether the element is
present (add) or removed (remove).}
...
\section{AI usage and application}
...
\begin{thebibliography}{00}
  ...
  \emph{ACM SIGACT News}, vol.~33, no.~2, pp.~51--59, Jun.\ 2002.
