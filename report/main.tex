...
A design decision was taken such that all protobuf conversion
...
interruption, it waits three seconds and tries to re-establish the stream,
performing a Sync call first to replay any entries missed during the outage. gRPC
...
can happen that 2 nodes send a Sync request at the same instant; each node takes a snapshot
...
This is solvable but out of scope for this project (e.g. Jitter, follower/leader Sync, Merkle tree, Delta state, etc.).
...
There are also 2 known limitations called the sputtering cycle, determined by
