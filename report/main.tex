...
string used as tiebreaker. \ref{merge-semantics}. This ensures \texttt{Before()}
...
of local state and send it, then upon receiving the snapshot both nodes update their local entries, both send the updates
...
After a week of downtime a node supposedly needs a re-initiation.
...
This is solved by forcing the node to re-initialize with empty settings whether
...
\texttt{log.Fatal} in the application entry-point. Forceful init had ben added
for nodes rejoining the cluster after more than TTL elapsed since last shutdown. A full
reconciliation cycle had ben added upon start to make sure ungraceful shutdowns did not
