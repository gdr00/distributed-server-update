...
\paragraph{Sender clock too far ahead} (e.g.\ B at $+6s$, \textbf{threshold}
$5s$, in our case $1s$): every correctly synchronised node rejects the entry.
B's writes are dropped cluster-wide until its NTP is corrected. No divergence occurs

\paragraph{Receiver clock too far behind} (e.g.\ A entrring with $-3s$, B entering with
$+4s$): from A's perspective the skew is $7s > 5s$, so A rejects,
from a synchronised node/cluster C the skew is $4s < 5s$, so C accepts and updates the clock to B wall skewing the whole cluster HLC to B's $+4s$ untill
next NTP correction and cluster making up for those seconds.
A and C temporarily diverge. Once A's clock is corrected, the next anti-entropy
round syncs A to the node/cluster C + B.
The divergence window is bounded by the NTP correction time, not permanent.

The $1s$ threshold is an operational boundary that is within the 100ms NTP average skew.
Nodes whose NTP drift pass it, diverge until clock correction, this is
